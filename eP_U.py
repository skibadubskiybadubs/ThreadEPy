...
def backup_output_controls(idf_file):
    ...
    """
    from eP_IDF import get_idf_manager
    manager = get_idf_manager()
    if manager:
        return manager.backup_output_controls(idf_file)
    else:
        print("Warning: IDF manager not initialized")
        return (False, None)
...
def inject_output_controls(idf_file, output_mode, output_files=None):
    ...
    manager = get_idf_manager()
    if manager:
        manager.inject_output_controls(idf_file, output_mode, output_files)
    else:
        print("Warning: IDF manager not initialized")
...
def restore_output_controls(idf_file, had_controls, original_text):
    ...
    manager = get_idf_manager()
    if manager:
        manager.restore_output_controls(idf_file, had_controls, original_text)
    else:
        print("Warning: IDF manager not initialized")
...
def restore_all_idf_files():
    ...
    manager = get_idf_manager()
    if manager:
        return manager.restore_all_idf_files()
    else:
        print("Warning: IDF manager not initialized")
        return 0
