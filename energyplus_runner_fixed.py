...
def check_and_install_dependencies():
    ...
    # Try to install optional dependencies for icon extraction
    try:
        import PIL
        ...
        try:
            print("Installing optional dependencies for icon support...")
            if os.name == 'nt':
                subprocess.check_call([sys.executable, "-m", "pip", "install", "Pillow", "pywin32"])
            else:
                subprocess.check_call([sys.executable, "-m", "pip", "install", "Pillow"])
            print("Icon support dependencies installed successfully.")
        except:
            print("Warning: Could not install icon support dependencies. Icons will be disabled.")
...
def extract_icon_from_executable(exe_path, output_path=None):
    """
    Extract icon from executable file and save as ICO file for use with tkinter.
    
    Args:
        exe_path (str): Path to the executable file
        output_path (str, optional): Path to save the icon file. If None, creates a temp file.
    
    Returns:
        str: Path to the extracted icon file, or None if extraction failed
    """
    if not os.path.exists(exe_path):
        return None
        
    if output_path is None:
        output_path = tempfile.mktemp(suffix='.ico', prefix='ep_icon_')
    ...
    try:
        if os.name == 'nt':  # Windows
            try:
                ...
                if large:
                    ...
                    # Draw the icon
                    win32gui.DrawIconEx(hdc_mem, 0, 0, large[0], ico_x, ico_y, 0, None, win32con.DI_NORMAL)
                    
                    # Convert to PIL Image and save as ICO
                    bmpstr = win32gui.GetBitmapBits(hbmp, ico_x * ico_y * 4)
                ...
                print(f"Failed to extract icon using win32: {e}")
                
            # Fallback method using simpler approach
            try:
                import win32gui
                # Try to extract using a simpler method
                large, small = win32gui.ExtractIconEx(exe_path, 0)
                if large:
                    # Save the icon handle for later use
                    win32gui.DestroyIcon(large[0])
                    if small:
                        win32gui.DestroyIcon(small[0])
                    # For simplicity, copy the exe file itself as tkinter can sometimes use it
                    import shutil
                    try:
                        shutil.copy2(exe_path, output_path.replace('.ico', '.exe'))
                        return output_path.replace('.ico', '.exe')
                    except:
                        pass
            except:
                pass
        ...
        print("Icon extraction dependencies not available. Skipping icon extraction.")
    except Exception as e:
        print(f"Failed to extract icon from {exe_path}: {e}")
    
    return None

def find_energyplus_icon(eplus_path):
    """
    Find and extract icon from EnergyPlus installation directory.
    ...
    """
    if not eplus_path or not os.path.exists(eplus_path):
        return None
    
    # List of possible EnergyPlus executables to try (in order of preference)
    possible_executables = [
        'eP_P.exe',           # Specific file mentioned by user
        'energyplus.exe',     # Main EnergyPlus executable
        'EP-Launch.exe',      # EnergyPlus launcher
        'EPDrawGUI.exe',      # EnergyPlus drawing tool
        'PreProcess\\EPMacro.exe',  # EnergyPlus preprocessor
    ]
    
    for exe_name in possible_executables:
        exe_path = os.path.join(eplus_path, exe_name)
        if os.path.exists(exe_path):
            print(f"Attempting to extract icon from: {exe_path}")
            icon_path = extract_icon_from_executable(exe_path)
            if icon_path:
                print(f"Successfully extracted icon: {icon_path}")
                return icon_path
        else:
            # Also try without subdirectory for some executables
            simple_exe_name = os.path.basename(exe_name)
            exe_path = os.path.join(eplus_path, simple_exe_name)
            if os.path.exists(exe_path):
                print(f"Attempting to extract icon from: {exe_path}")
                icon_path = extract_icon_from_executable(exe_path)
                if icon_path:
    ...
    print("No suitable EnergyPlus executable found for icon extraction.")
    return None

# Global variable to store the application icon path
APP_ICON_PATH = None

def get_app_icon(eplus_path=None):
    """
    Get the application icon path, extracting it from EnergyPlus if needed.
    
    Args:
        eplus_path (str, optional): Path to EnergyPlus installation
    
    Returns:
        str: Path to icon file, or None if not available
    """
    ...
    # Try to extract from EnergyPlus installation
    if eplus_path:
        APP_ICON_PATH = find_energyplus_icon(eplus_path)
        if APP_ICON_PATH:
            return APP_ICON_PATH
    
    # Try default EnergyPlus path
    if os.path.exists(DEFAULT_EPLUS_PATH):
        APP_ICON_PATH = find_energyplus_icon(DEFAULT_EPLUS_PATH)
        if APP_ICON_PATH:
            return APP_ICON_PATH
    
    return None

def set_console_icon(icon_path=None):
    """
    Set the console window icon (Windows only).
        ...
        if icon_path and os.path.exists(icon_path):
            ...
            if hwnd:
                ...
                if icon_path.endswith('.exe'):
                    # Extract icon from executable
                    hicon = user32.ExtractIconW(kernel32.GetModuleHandleW(None), icon_path, 0)
                elif icon_path.endswith('.ico'):
                    # Load from ICO file
                    hicon = user32.LoadImageW(None, icon_path, 1, 0, 0, 0x00000010)  # LR_LOADFROMFILE
                
                if hicon:
                    # Set both small and large icons
                    user32.SendMessageW(hwnd, 0x0080, 0, hicon)  # WM_SETICON, ICON_SMALL
                    user32.SendMessageW(hwnd, 0x0080, 1, hicon)  # WM_SETICON, ICON_LARGE
                    print(f"Console icon set successfully from: {icon_path}")
                else:
                    print(f"Failed to load icon from: {icon_path}")
    except Exception as e:
        print(f"Failed to set console icon: {e}")
...
class EnergyPlusGUI:
    ...
    def set_window_icon(self, eplus_path=None):
        """Set the window icon from EnergyPlus installation"""
        try:
            if not eplus_path:
                eplus_path = self.eplus_folder.get() if hasattr(self, 'eplus_folder') else None
            
            icon_path = get_app_icon(eplus_path)
            if icon_path and os.path.exists(icon_path):
                if icon_path.endswith('.ico'):
                    self.root.iconbitmap(icon_path)
                    print(f"GUI icon set successfully from: {icon_path}")
                elif icon_path.endswith('.exe'):
                    # For exe files, try to use them directly (Windows can sometimes extract icons)
                    try:
                        self.root.iconbitmap(default=icon_path)
                        print(f"GUI icon set successfully from executable: {icon_path}")
                    except:
                        # If that fails, try to extract the icon first
                        ico_path = extract_icon_from_executable(icon_path)
                        if ico_path and ico_path.endswith('.ico'):
                            self.root.iconbitmap(ico_path)
                            print(f"GUI icon set successfully from extracted icon: {ico_path}")
        except Exception as e:
            print(f"Failed to set GUI icon: {e}")
    ...
    def setup_dark_theme(self):
        ...
        self.style.configure('Banner.TLabel', background=self.colors['banner_bg'], foreground=self.colors['banner_fg'], 
                            font=('Arial', banner_font_size, 'bold'))
    ...
    def create_widgets(self):
        ...
        # Update icon with default path
        self.set_window_icon(DEFAULT_EPLUS_PATH)
    ...
    def select_eplus_folder(self):
        ...
        if folder:
            ...
            # Update window icon with the new EnergyPlus path
            self.set_window_icon(folder)
...
def cleanup_and_exit():
    ...
    try:
        ...
        global APP_ICON_PATH
        if APP_ICON_PATH and os.path.exists(APP_ICON_PATH) and 'temp' in APP_ICON_PATH.lower():
            try:
                os.unlink(APP_ICON_PATH)
            except:
                pass
...
def main():
    ...
    if args.run_simulations:
        ...
        # Set console icon if possible
        set_console_icon(get_app_icon(config.get('eplus_path')))
    ...
    if len(sys.argv) == 1: # GUI mode
        ...
        # Set console icon if possible
        set_console_icon(get_app_icon(args.eplus))
...
if __name__ == "__main__":
    main(), 'r')
        ...
        # Set console title
        kernel32.SetConsoleTitleW(f"{APP_NAME} - Simulation Console")
